...
import com.heziz.liyang.adaper.smz.MySMZExtendableListViewAdapter;
...
import com.heziz.liyang.bean.smz.SMZBean;
...
public class SmzProjectListActivity extends BaseActivity implements View.OnClickListener {
    ...
    private String name;
    MySMZExtendableListViewAdapter adapter;
    List<SMZBean> list=new ArrayList<>();
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_smz_project_list);
    ...
    private void initViews() {
        ...
        adapter=new MySMZExtendableListViewAdapter(this,list);
...
//        获取街道信息
    ...
    private void initProjectData() {
        showProgressDialog();
        String url1 = API.SMZ_PROJECT_LIST+"?access_token="+ MyApplication.getInstance().getUserInfor().getUuid();

        JsonCallBack1<SRequstBean<RequestBean<List<SMZBean>>>> jsonCallBack1 = new JsonCallBack1<SRequstBean<RequestBean<List<SMZBean>>>>() {
            @Override
            public void onSuccess(com.lzy.okgo.model.Response<SRequstBean<RequestBean<List<SMZBean>>>> response) {
                list.clear();
...
//                showMark(projectBeanList);
            ...
            @Override
            public void onError(com.lzy.okgo.model.Response<SRequstBean<RequestBean<List<SMZBean>>>> response) {
                super.onError(response);
                dissmissProgressDialog();
            }
...
//                Toast.makeText(getApplicationContext(), groupString[groupPosition], Toast.LENGTH_SHORT).show();
                if(!parent.isGroupExpanded(groupPosition)){
                    if(list.get(groupPosition).getDevices().size()==0){

...
import com.heziz.liyang.ui.zhihui.smz.SmzDetailsActivity;
...
public class MySMZExtendableListViewAdapter extends BaseExpandableListAdapter {

    private Context mContext;
    private List<SMZBean> list;
    public MySMZExtendableListViewAdapter(Context mContext, List<SMZBean> list) {
    ...
    public int getChildrenCount(int groupPosition) {
        return list.get(groupPosition).getDevices().size();
    }
    ...
    public Object getChild(int groupPosition, int childPosition) {
        return list.get(groupPosition).getDevices().get(childPosition);
    }
...
//        num.setText(list.get(groupPosition).getAvgValue());
    ...
    public View getChildView(int groupPosition, int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
            ...
            tvDeviceId.setText(list.get(groupPosition).getDevices().get(childPosition).getDeviceNo());
            int status=list.get(groupPosition).getDevices().get(childPosition).getState();
            //btn.setText(status);
            if(status==1){
                btn.setText("在线");
                btn.setEnabled(true);
            ...
            btn.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    Intent intent=new Intent(mContext,SmzDetailsActivity.class);
                    intent.putExtra("id",list.get(groupPosition).getDevices().get(childPosition).getDeviceNo());
                    intent.putExtra("proid",list.get(groupPosition).getId());

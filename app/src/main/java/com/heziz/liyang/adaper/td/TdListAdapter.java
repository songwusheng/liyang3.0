...
public class TdListAdapter extends BaseQuickAdapter<TdStreetDataBean,BaseViewHolder> {
    private Context context;
    public TdListAdapter(Context context, List<TdStreetDataBean> list) {
        super(R.layout.zh_sp_list_include1,list);
        this.context=context;
    }

    @Override
    protected void convert(BaseViewHolder helper, TdStreetDataBean item) {
        helper.setText(R.id.tvJd, item.getJdName());
        helper.setText(R.id.tvNum, helper.getLayoutPosition()+1+"");
        helper.setText(R.id.tvTotal,item.getCount()+"");

...
import com.heziz.liyang.utils.HighlightCR;
...
public class DashboardView extends View {
    ...
    private long duration = 50; // 动画默认时长
    ...
    public DashboardView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mMeasureTextSize = a.getDimensionPixelSize(R.styleable.DashboardView_measureTextSize, spToPx(10));
    ...
    private String[] getMeasureNumbers() {
        ...
        for (int i = 0; i <= mBigSliceCount; i++) {
            if (i == 0) {
                ...
                strings[i] = String.valueOf(mMinValue+45*i);
            }
        }
    ...
    private void initSizes() {
        ...
        mSmallSliceRadius = mRadius - dpToPx(15);
        mBigSliceRadius = mSmallSliceRadius - dpToPx(4);
        mNumMeaRadius = mBigSliceRadius - dpToPx(4);

        mSmallSliceCount = mBigSliceCount * mSliceCountInOneBigSlice;
        //刻度跨度比弧度小30度
        mBigSliceAngle = (mSweepAngle-30) / (float) mBigSliceCount;
    ...
    private void drawMeasures(Canvas canvas) {
        ...
        for (int i = 0; i <= mBigSliceCount; i++) {
            //绘制大刻度
            float angle = i * mBigSliceAngle + mStartAngle+15;
            float[] point1 = getCoordinatePoint(mRadius-8, angle);
        ...
        for (int i = 0; i < mSmallSliceCount; i++) {
            if (i % mSliceCountInOneBigSlice != 0) {
                float angle = i * mSmallSliceAngle + mStartAngle+15;
                float[] point1 = getCoordinatePoint(mRadius-15, angle);
    ...
    private void drawCircleAndReadingText(Canvas canvas) {
        ...
        mPaintPointer.setColor(getResources().getColor(R.color.maincolor));
        ...
        // 绘制读数
        canvas.drawText("回旋"+trimFloat(mRealTimeValue)+"°", mCenterX,
                mCenterY + mCircleRadius + dpToPx(2) + dpToPx(45), mPaintValue);
    }
    ...
    private float getAngleFromResult(float result) {
        if (result > mMaxValue)
            return mMaxValue;
        return (mSweepAngle-30 )* (result - mMinValue) / (mMaxValue - mMinValue) + mStartAngle+15;
    }
    ...
    private class MyHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == 0) {
                ...
                if (Math.abs(preValue - endValue) > 10) {
